...
public class Model extends AGModel {
	...
	public void createBuffers(GL gl) {
		gl._gl.glGenBuffers(buffers.length, buffers, 0);
	}

	public void updateBuffers(GL gl, boolean forced) {
		if (forced || (updateKey != updateState)) {
			floatBuffer.position(0);
			gl._gl.glBindBuffer(GL._GL.GL_ARRAY_BUFFER, buffers[0]);
			gl._gl.glBufferData(GL._GL.GL_ARRAY_BUFFER, floatSize * 4, floatBuffer.floats, GL._GL.GL_DYNAMIC_DRAW);
	...
	public void destroyBuffers(GL gl) {
		gl._gl.glDeleteBuffers(buffers.length, buffers, 0);
	}
